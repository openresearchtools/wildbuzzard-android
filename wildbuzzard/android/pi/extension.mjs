...
export default function wildBuzzard(pi) {
  pi.registerTool({
    ...
    promptGuidelines: ['Use wildbuzzard_browser snapshots to obtain element references before acting; take a fresh snapshot after navigation.',
      'For act: click/focus/check/uncheck use target; fill uses target,value,clear; type uses text; press uses key (for example Enter); select uses target,value; scroll uses direction (up/down/left/right) and amount (120-pixel steps).',
      'Navigation starts asynchronously. Inspect a fresh snapshot or wait for a known page condition before acting again. A truncated snapshot can be narrowed with target from a container in that snapshot.',
