...
def main():
    ...
    manifest_path.write_text(json.dumps({'description':'Original-resource audit; generated app overrides replace upstream illustration and icon geometry. Structural shapes/selectors retain upstream licenses. Website content and Android framework controls are outside app branding.','brand_aliases':sorted(aliases),'resources':dict(sorted(manifest.items()))},indent=2)+'\n')
