...
public final class OnionActivity extends ProductActivity {
    ...
    @Override public void onCreate(Bundle saved) {
        ...
        TextView description = new TextView(this); description.setText("Scan the TorKitten / Orbot credential QR or choose its .auth_private file. Each contains both the onion address and the private key. Keys stay encrypted on this device."); root.addView(description);
        ...
        add(manual, "Import key", () -> { try { save(new OnionKey(address.getText().toString(), secret.getText().toString())); } catch (Exception error) { message(error.getMessage()); } });
        ...
        add(root, "Remove an imported key", () -> app.tor.list(hosts -> new AlertDialog.Builder(this).setTitle("Remove key")
            .setItems(hosts.toArray(new String[0]), (d, i) -> app.tor.remove(hosts.get(i), value -> { message(value); refreshImported(); })).show()));
        add(root, "Back to browser", this::finish);
    ...
    private void refreshImported() {
        app.tor.list(hosts -> {
            ...
            TextView label = new TextView(this); label.setText("Imported private sites"); imported.addView(label);
            for (String host : hosts) add(imported, "Open " + host, () ->
                app.create(BrowserApp.USER, true, "https://" + host + "/", app::show, this::message));
        });
    }
