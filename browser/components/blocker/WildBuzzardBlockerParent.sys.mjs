 ...
 */
export class WildBuzzardBlockerParent extends JSWindowActorParent {
  /**
  ...
  receiveMessage(message) {
    switch (message.name) {
      case "WildBuzzardBlocker:GetCosmeticResources":
        return this._getCosmeticResources(message.data);
      case "WildBuzzardBlocker:GetHiddenClassIdSelectors":
  ...
  async _getCosmeticResources({ url } = {}) {
    ...
    await WildBuzzardBlockerService.whenEngineReady();

    const resources = WildBuzzardBlockerService.getCosmeticResources(
      url,
  ...
  _getHiddenClassIdSelectors({ classes, ids, exceptions } = {}) {
    if (!WildBuzzardBlockerService.isAndroidTabBlockingEnabled(this.browsingContext?.top?.browserId)) return [];
