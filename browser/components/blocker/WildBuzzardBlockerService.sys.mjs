...
const PREF_LEGACY_SITE_EXCEPTIONS = "wildbuzzard.blocker.siteExceptions";
const PREF_SITE_EXCEPTIONS_MIGRATED =
  "wildbuzzard.blocker.siteExceptions.migrated";
...
function makeUnicodeConverter(charset) {
  /* eslint-disable mozilla/reject-scriptableunicodeconverter --
   * Response rewriting must round-trip legacy HTTP character sets; TextEncoder
   * can only emit UTF-8. */
  const converter = Cc[
    "@mozilla.org/intl/scriptableunicodeconverter"
  ].createInstance(Ci.nsIScriptableUnicodeConverter);
  /* eslint-enable mozilla/reject-scriptableunicodeconverter */
...
function applyReplaceDirectives(text, directives) {
  ...
  for (const directive of directives) {
    try {
      ...
      console.warn(
        "[WildBuzzardBlocker] Skipping invalid $replace directive:",
        err
      );
    }
  }
...
function applyHtmlFilters(text, filters, documentUrl, mimeType) {
  ...
  for (const rawFilter of filters) {
    ...
    try {
      ...
      console.warn("[WildBuzzardBlocker] Skipping invalid HTML filter:", err);
      continue;
    }
  ...
  onStopRequest(request, statusCode) {
    if (!this.passThrough) {
      ...
      try {
        ...
        console.warn(
          "[WildBuzzardBlocker] Failed to filter response body:",
          err
        );
        this.emitBytes(request, originalBytes);
      }
...
export const WildBuzzardBlockerService = {
  ...
  _listUpdates() {
    if (!this._listUpdatesState) {
      this._listUpdatesState = new ListUpdatesState();
    }
  ...
  _createEngine() {
    return Cc[CONTRACT_ID].createInstance(Ci.nsIWildBuzzardBlockerEngine);
  },
    ...
    return lazy.ListStore.withListWriteLock(async () => {
      ...
      for (const descriptor of descriptors) {
        ...
        try {
          ...
          console.warn(
            `[WildBuzzardBlocker] Failed to fetch list: ${descriptor.url}`,
            err
          );
        }
      }
  ...
  async _preprocessListRecords(listRecords) {
    const records =
      await lazy.ListStore.withWildBuzzardUnbreakRecord(listRecords);
  ...
  async _initFromTextSourcesAndCache(descriptors, generation) {
    ...
    if (storedListsUsable) {
      try {
        ...
        console.warn(
          "[WildBuzzardBlocker] Stored lists failed to load, trying network fetch:",
          err
        );
      }
    }
    ...
    if (fetchedListsUsable) {
      try {
        ...
        console.warn(
          "[WildBuzzardBlocker] Fetched lists failed to load, falling back to bundled:",
          err
        );
      }
    }
  ...
  async _doInitializeEngineIfNeeded() {
    ...
    try {
      ...
      if (e.result !== Cr.NS_ERROR_FILE_NOT_FOUND) {
        console.error("[WildBuzzardBlocker] Unexpected cache error:", e);
      }
    }
  ...
  _notifyBlockedCountsCleared() {
    try {
      ...
      console.warn(
        "[WildBuzzardBlocker] Failed to notify cleared counts:",
        err
      );
    }
  },
  ...
  _notifyBlockedCountUpdated(browserId, blockedCount) {
    try {
      ...
      console.warn("[WildBuzzardBlocker] Failed to notify blocked count:", err);
    }
  },
  ...
  _handleTopLevelDocumentRequest(
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlocker] Failed to redirect to blocked page:",
        err
      );
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlocker] Failed to increment blocked count:",
        err
      );
    }
  },
  ...
  _redirectChannel(channel, target) {
    try {
      ...
      console.warn("[WildBuzzardBlocker] Failed to redirect channel:", err);
      return false;
    }
  },
  ...
  _onExamineResponse(subject) {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlocker] Failed to apply CSP directives:",
        err
      );
    }
  },
  ...
  _maybeFilterResponseBody(
    ...
    try {
      ...
      console.warn(
        "[WildBuzzardBlocker] Failed to install response filter:",
        err
      );
    }
  },
  ...
  /**
   * Schedules a startup catch-up and a 24-hour refresh interval independently
   * of Firefox Remote Settings or a vendor update service.
   */
  _startListUpdateTriggers() {
    ...
    this._startupCatchupTimerId = lazy.setTimeout(() => {
      ...
      this._updateListsIfNeeded().catch(err => {
        console.warn("[WildBuzzardBlocker] Startup list update failed:", err);
      });
    }, STARTUP_LIST_CATCHUP_DELAY_MS);

    this._fallbackUpdateIntervalId = lazy.setInterval(() => {
      this._updateListsIfNeeded().catch(err => {
        console.warn("[WildBuzzardBlocker] Fallback list update failed:", err);
      });
  ...
  /**
   * Refreshes lists, then rebuilds the engine when content changed.
   * Conditional requests are cheap enough that callers provide the cadence
   * instead of using a separate staleness gate here.
   */
  ...
  checkRequest(url, sourceHostname, hostname, requestType, isThirdParty) {
    ...
    try {
      ...
      console.error("[WildBuzzardBlocker] checkRequest failed:", err);
      return this._normalizeCheckResult(null);
    }
  },
  ...
  getCosmeticResources(url, browsingContext = null) {
    ...
    try {
      ...
      console.error("[WildBuzzardBlocker] getCosmeticResources failed:", err);
      return null;
    }
  ...
  getCspDirectives(url, sourceHostname, hostname, requestType, isThirdParty) {
    ...
    try {
      ...
      console.error("[WildBuzzardBlocker] getCspDirectives failed:", err);
      return "";
    }
  },
  ...
  getReplaceDirectives(
    ...
    try {
      ...
      console.error("[WildBuzzardBlocker] getReplaceDirectives failed:", err);
      return [];
    }
  },
  ...
  getHtmlFilteringResources(url) {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlocker] getHtmlFilteringResources failed:",
        err
      );
      return { htmlFilters: [], responseHeaderFilters: [] };
    }
  ...
  async setCustomFiltersText(text) {
    ...
    if (hadPreviousFile) {
      try {
        ...
        console.warn(
          "[WildBuzzardBlocker] Failed reading previous custom filters for rollback:",
          err
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.error(
          "[WildBuzzardBlocker] Failed rolling back custom filters after rebuild failure:",
          rollbackErr
        );
      }
  ...
  getHiddenClassIdSelectors(classes = [], ids = [], exceptions = []) {
    ...
    try {
      ...
      console.error(
        "[WildBuzzardBlocker] getHiddenClassIdSelectors failed:",
        err
      );
      return [];
    }
  ...
  _unregisterNetworkObservers() {
    ...
    for (const topic of [
      ...
      try {
        ...
        console.warn(
          `[WildBuzzardBlocker] Failed to remove observer for ${topic}:`,
          err
        );
      }
    }
  ...
  async _initializeEngineWithRetry() {
    try {
      ...
      console.error("[WildBuzzardBlocker] Failed to initialise engine:", err);
  ...
  _migrateSiteExceptions() {
    ...
    if (raw) {
      try {
        ...
        console.warn(
          "[WildBuzzardBlocker] Failed to parse legacy site exceptions pref:",
          err
        );
      }
    }
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.warn(
          `[WildBuzzardBlocker] Failed to migrate site exception "${entry}":`,
          err
        );
      }
    }
  ...
  async init() {
    ...
    lazy.EngineCache.cleanupStale().catch(err => {
      console.warn("[WildBuzzardBlocker] Cache cleanup failed:", err);
    });
    ...
    switch (data) {
      ...
      case PREF_FILTER_LIST_URLS:
        if (this.isEnabled()) {
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[WildBuzzardBlocker] Failed to refresh lists after pref change:",
              err
            );
          });
        }
      ...
      case PREF_ENABLED_LISTS:
        if (this.isEnabled()) {
          ...
          this.refreshListsAndEngine().catch(err => {
            console.error(
              "[WildBuzzardBlocker] Failed to refresh lists after list toggle:",
              err
            );
          });
        }
  ...
  shouldBypassBlocking(candidateDomain, options = {}) {
    ...
    return this.isSiteExcepted(domain, {
  ...
  uninit() {
    ...
    try {
      ...
      console.warn("[WildBuzzardBlocker] Failed to remove pref observer:", err);
    }
